...
unsigned Hash::Buscar(const std::string& chave) const {
    ...
    do
    {
        ...
        if (dado == nullptr)
        {
            //cout << "Busca: chave ta vazia"<<endl;
            return -1;
        }
        if (dado->mChave == chave)
        {
           // cout << "Busca: achou a chave"<<endl;
            return pos;
        }
        pos = (pos+1)%mCapacidade;
        
    } while (pos != posFinal);
    //cout << "Busca: Nao achou a chave"<<endl;
    return -1;
}
...
void Hash::Remover(const std::string& chave) {
    ...
    if (mTamanho == 0)
    {
       // cout <<"Remover: Tabela vazia"<<endl;
        throw invalid_argument("");
    }
    int pos = Buscar(chave);
    if (pos == -1)
    {
       // cout <<"Remover: Chave nao encontrada"<<endl;
        throw invalid_argument("");
    }
